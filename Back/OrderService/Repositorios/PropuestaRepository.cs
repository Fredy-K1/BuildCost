...
namespace OrderService.Repositorios
{
    public class PropuestaRepository : IPropuestaRepository
    {
        ...
        public async Task<IEnumerable<Propuesta>> GetByContratistaIdAsync(Guid contratistaId)
        {
            return await _appDbContext.Propuestas
                                      .Where(p => p.ContratistaId == contratistaId)
                                      .ToListAsync();
        }
        ...
        public async Task<Propuesta?> UpdateAsync(int propuestaId, UpdatePropuestaDto updatePropuestaDto)
        {
            ...
            // Validación segura para los strings
            if (!string.IsNullOrWhiteSpace(updatePropuestaDto.Detalles))
            {
                propuesta.Detalles = updatePropuestaDto.Detalles;
            }

            if (!string.IsNullOrWhiteSpace(updatePropuestaDto.PdfUrl))
            {
                propuesta.PdfUrl = updatePropuestaDto.PdfUrl;
            }
